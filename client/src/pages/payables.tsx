...
export default function PayablesPage() {
  ...
  const { data, isLoading } = useQuery<any[]>({
    queryKey: ["/api/outstanding/payables"],
  });

  const paymentMutation = useMutation({
    mutationFn: (d: any) => apiRequest("POST", "/api/payments", d),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/outstanding/payables"] });
  ...
  return (
    <div className="p-6 space-y-6 overflow-auto h-full" dir="rtl">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold flex items-center gap-2" data-testid="text-payables-title">
          <AlertTriangle className="w-6 h-6 text-orange-500" /> المستحقات للموردين
        </h1>
        <Card className="px-4 py-2">
          <div className="text-sm text-muted-foreground">إجمالي المطلوبات</div>
          <div className="text-xl font-bold text-red-600" data-testid="text-total-payables">{totalOutstanding.toLocaleString()} ج.م</div>
        </Card>
      </div>
      ...
      {(!data || data.length === 0) ? (
        <Card>
          <CardContent className="py-12 text-center text-muted-foreground">
            ...
            <p>لا توجد مطلوبات متأخرة</p>
          </CardContent>
        </Card>
      ) : (
        data.map((group: any) => (
          <Card key={group.supplier.id} data-testid={`card-supplier-${group.supplier.id}`}>
            <CardHeader className="pb-2">
              <CardTitle className="flex items-center justify-between">
                <Link href={`/suppliers/${group.supplier.id}/statement`} className="text-primary hover:underline flex items-center gap-2">
                  <Truck className="w-5 h-5" /> {group.supplier.name}
                </Link>
              ...
              <Table>
                ...
                <TableBody>
                  {group.invoices.map((inv: any) => (
                    <TableRow key={inv.id} data-testid={`row-payable-${inv.id}`}>
                      ...
                      <TableCell>
                        ...
                        }} data-testid={`button-pay-payable-${inv.id}`}>
