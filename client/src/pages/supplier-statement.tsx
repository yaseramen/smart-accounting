...
import { ArrowRight, Truck, FileText, TrendingUp, TrendingDown, Scale, CreditCard } from "lucide-react";
...
export default function SupplierStatementPage() {
  ...
  const buildUrl = () => {
    ...
    return `/api/suppliers/${supplierId}/statement${qs ? `?${qs}` : ""}`;
  };

  const { data, isLoading } = useQuery<any>({
    queryKey: ["/api/suppliers", supplierId, "statement", startDate, endDate],
  ...
  const paymentMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/suppliers", supplierId, "statement"] });
  ...
  if (!data?.supplier) {
    return <div className="p-6" dir="rtl"><p className="text-muted-foreground">المورد غير موجود</p></div>;
  }

  const { supplier, invoices, summary } = data;

  const getTypeBadge = (type: string) => {
    if (type === "purchase") return <Badge variant="default">فاتورة شراء</Badge>;
    if (type === "purchase_return") return <Badge variant="secondary">مرتجع شراء</Badge>;
  ...
  return (
    <div className="p-6 space-y-6 overflow-auto h-full" dir="rtl">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold flex items-center gap-2" data-testid="text-supplier-statement-title">
          <Truck className="w-6 h-6" /> كشف حساب: {supplier.name}
        </h1>
      </div>
            ...
            <TableBody>
              {invoices?.map((inv: any) => (
                <TableRow key={inv.id} data-testid={`row-statement-inv-${inv.id}`}>
                  ...
                  <TableCell>
                    {inv.type === "purchase" && (inv.paymentType === "deferred" || inv.paymentType === "partial") && parseFloat(inv.outstanding) > 0 && (
      ...
      <div className="grid grid-cols-2 gap-4 text-sm">
        ...
        <div><strong>البريد:</strong> {supplier.email || "-"}</div>
        <div><strong>العنوان:</strong> {supplier.address || "-"}</div>
        <div><strong>الرقم الضريبي:</strong> {supplier.taxNumber || "-"}</div>
      </div>
